...
class PatternTest < ActiveSupport::TestCase
    ...
    test 'determine_file_extension_of_auidio, should return proper extension for a given codec' do 
        some_pattern = Pattern.new(project: projects(:main_one))
        extension = some_pattern.determine_file_extension_of_auidio("#{Rails.root}/test/fixtures/test.mp4")
        assert "m4a", extension  
    end
